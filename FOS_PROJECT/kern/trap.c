...
struct WorkingSetElement * find_in_sc_list(struct Env * curenv, uint32 va) {
	struct WorkingSetElement *element;
	LIST_FOREACH(element, &(curenv->SecondList))
	{
		if (element->virtual_address == va) {
			return element;
		}
	}
	return NULL;
...
void apply_placement(struct Env * curenv, uint32 fault_va) {
	int va_is_found_in_sc = 0;
	struct WorkingSetElement *element = find_in_sc_list(curenv, fault_va);
	...
	if (!va_is_found_in_sc) {
		...
		map_frame((curenv->env_page_directory), frame_info, (void*) fault_va, PERM_USER | PERM_PRESENT | PERM_WRITEABLE);
		int page_exist = pf_read_env_page(curenv, (void*) fault_va);
		if (page_exist == E_PAGE_NOT_EXIST_IN_PF){
			if (fault_va >= USTACKBOTTOM && fault_va < USTACKTOP) {
				pf_add_empty_env_page(curenv, fault_va, (uint8)0);
			} else {
...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	if (LIST_SIZE(&(curenv->PageWorkingSetList)) != 0 || find_in_sc_list(curenv, fault_va) != NULL) {
		apply_placement(curenv, fault_va);
	} else {
		struct WorkingSetElement *lastElement;
		if (curenv->SecondListSize == 0) {
			lastElement = LIST_LAST(&(curenv->ActiveList));
			LIST_REMOVE(&(curenv->ActiveList), lastElement);
		} else {
			lastElement = LIST_LAST(&(curenv->SecondList));
			LIST_REMOVE(&(curenv->SecondList), lastElement);
		}
		uint32 page_permissions = pt_get_page_permissions(curenv, lastElement->virtual_address);
		if(page_permissions & PERM_MODIFIED){
			uint32 *ptr_page_table;
			struct Frame_Info* frame_info = get_frame_info(curenv->env_page_directory, (void *) lastElement->virtual_address, &ptr_page_table);
			pf_update_env_page(curenv, (void *) lastElement->virtual_address, frame_info);
		}
		unmap_frame(curenv->env_page_directory, (void *) lastElement->virtual_address);
		lastElement->empty = 1;
		pt_set_page_permissions(curenv, lastElement->virtual_address, 0, PERM_PRESENT);
		LIST_INSERT_HEAD(&(curenv->PageWorkingSetList), lastElement);
		apply_placement(curenv, fault_va);
	}
