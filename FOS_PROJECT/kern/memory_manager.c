...
int freeList(struct Env* e, struct WS_List *list, uint32 virtual_address, uint32 max_virtual_address){
	struct WorkingSetElement *element;
	LIST_FOREACH(element,list){
		uint32 va_of_elem = element->virtual_address;
		if(va_of_elem == virtual_address){
			pt_set_page_permissions(e, element->virtual_address,
				0, PERM_USER | PERM_PRESENT | PERM_WRITEABLE);
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	int mx_size = e->page_WS_max_size;
	for(int i = virtual_address; i < va; i += PAGE_SIZE){
		for(int j = 0; j < mx_size; j++){
			if(i == env_page_ws_get_virtual_address(e, j)){
				struct WorkingSetElement *element;
				int ret = freeList(e, &(e->ActiveList), i, va);
				if(ret){
					if(LIST_SIZE(&(e->SecondList))){
						struct WorkingSetElement *firstelement = LIST_FIRST(&(e->SecondList));
						LIST_REMOVE(&(e->SecondList), firstelement);
						LIST_INSERT_TAIL(&(e->ActiveList), firstelement);
						pt_set_page_permissions(e, firstelement->virtual_address, PERM_PRESENT | PERM_WRITEABLE | PERM_USER, 0);
					}
				}
				freeList(e, &(e->SecondList), i, va);
		...
		int table_exist = get_page_table(e->env_page_directory, (void *) i, &pg_table);
		if(table_exist != TABLE_NOT_EXIST){
			...
			if(empty){
				uint32 *pg_table_new = NULL;
				struct Frame_Info *frame_ifno = get_frame_info(e->env_page_directory, (void *)pg_table, &pg_table_new);
				free_frame(frame_ifno);
